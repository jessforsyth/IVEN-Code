...
for file_num=1:num_files
    ...
    [~,~,~,~,~,~,~,~,~,~,~,OUT,nbrs_old,nbrs_new,nbr_comp,med_nbr_dist]=import_nbr_file(table,headings);
    
    ins_med_dist=[ins_med_dist;med_nbr_dist(logical(ones(num_cells,1)-OUT))];
    out_med_dist=[out_med_dist;med_nbr_dist(logical(OUT))];
    ins_nbrs=[];
    out_nbrs=[];
    mu_nbrs=[];
    po_nbrs=[];
